...
def pass_wheels(input, reverse = False):
    ...
    if not reverse:
        for index in range(len(SETTINGS["WHEELS"])):
            if index == 0:
                input = SETTINGS["WHEELS"][index]["wire"][ord(input) - ord('A')]
            else:
                input = SETTINGS["WHEELS"][index]["wire"][ord(input) - ord('A') - SETTINGS["WHEEL_POS"][index - 1]]
            # print(input, end=' -> ')
        return input
    else:
        output = 0
        for index in reversed(range(len(SETTINGS["WHEELS"]))):
            if index == 2:
                # print(chr(SETTINGS["WHEELS"][index]["wire"].index(input) + ord('A') + abs(SETTINGS["WHEEL_POS"][index] - SETTINGS["WHEEL_POS"][index - 1])))
                output = SETTINGS["WHEELS"][index]["wire"].index(input) + ord('A') + abs(SETTINGS["WHEEL_POS"][index] - SETTINGS["WHEEL_POS"][index - 1])
                output = output % ord('Z') + ord('A') - 1 if output > ord('Z') else output
                output = chr(output)
                # print(output, end=' -> ')
            elif index != 0:
                output = SETTINGS["WHEELS"][index]["wire"].index(output) + ord('A') + abs(SETTINGS["WHEEL_POS"][index] - SETTINGS["WHEEL_POS"][index - 1])
                output = output % ord('Z') + ord('A') - 1 if output > ord('Z') else output
                output = chr(output)
                # print(output, end=' -> ')
                
            else:
                output = SETTINGS["WHEELS"][index]["wire"].index(output) + ord('A')
                output = output % ord('Z') + ord('A') - 1 if output > ord('Z') else output
                output = chr(output)
                # print(output, end=' -> ')
...
# pass reflector
def pass_ukw(input):
    # print(SETTINGS["UKW"][ord(input) - ord('A')], end=' -> ')
    return SETTINGS["UKW"][ord(input) - ord('A')]

# Wheel Rotation
def rotate_wheels():
    # Implement Wheel Rotation Logics
    wheel_1 = SETTINGS["WHEELS"][0]
    SETTINGS["WHEEL_POS"][0] = (SETTINGS["WHEEL_POS"][0] + 1) % 26
    wheel_1["wire"] = wheel_1["wire"][1:] + wheel_1["wire"][0]

    if SETTINGS["WHEEL_POS"][0] == wheel_1["turn"]:
        wheel_2 = SETTINGS["WHEELS"][1]
        SETTINGS["WHEEL_POS"][1] = (SETTINGS["WHEEL_POS"][1] + 1) % 26
        wheel_2["wire"] = wheel_2["wire"][1:] + wheel_2["wire"][0]
        
        if SETTINGS["WHEEL_POS"][1] == wheel_2["turn"]:
            wheel_3 = SETTINGS["WHEELS"][2]
            SETTINGS["WHEEL_POS"][2] = (SETTINGS["WHEEL_POS"][2] + 1) % 26
            wheel_3["wire"] = wheel_3["wire"][1:] + wheel_3["wire"][0]
