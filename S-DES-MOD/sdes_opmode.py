...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    ...
    while encrypt_length < len(text):
        encrypt_split = sdes(text[encrypt_length : encrypt_length + 8], key, MODE_ENCRYPT)
        ciphertext.append(encrypt_split)
        encrypt_length += 8
...
def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    ...
    while decrypt_length < len(ciphertext):
        decrypt_split = sdes(ciphertext[decrypt_length : decrypt_length + 8], key, MODE_DECRYPT)
        plaintext.append(decrypt_split)
        decrypt_length += 8
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    while encrypt_length < len(text):
        text_split = text[encrypt_length : encrypt_length + 8]
        text_split = text_split ^ initialization_vector
        encyrpt_split = sdes(text_split, key, MODE_ENCRYPT)

        ciphertext.append(encyrpt_split)

        initialization_vector = encyrpt_split
        encrypt_length += 8
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    while decrypt_length < len(ciphertext):
        ciphertext_split = ciphertext[decrypt_length : decrypt_length + 8]
        decyrpt_split = sdes(ciphertext_split, key, MODE_DECRYPT)
        decyrpt_split = decyrpt_split ^ initialization_vector
    
        plaintext.append(decyrpt_split)

        initialization_vector = ciphertext_split
        decrypt_length += 8
