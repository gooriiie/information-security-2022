...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    left_half = input[0:4]
    right_half = input[4:8]

    # --------------- 0 round start ---------------------------

    # mode : 1 -> 0 / mode : 2 -> 1
    result_round_0 = round(right_half, keys[mode - 1])
    
    temp = right_half
    right_half = left_half ^ result_round_0
    left_half = temp

    # --------------- 1 round start ---------------------------

    # mode : 1 -> 1 / mode : 2 -> 0
    result_round_1 = round(right_half, keys[mode % 2])

    left_half ^= result_round_1
